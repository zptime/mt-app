  ...
  <div class="page-order">
    <el-row>
      <el-col :span="4" class="navbar">
        ...
        <dl>
          <dt>抵用券</dt>
          <dd>可用劵<i class="el-icon-arrow-right" /></dd>
          <dd>失效劵<i class="el-icon-arrow-right" /></dd>
        </dl>
        <dl>
          <dt>个人信息</dt>
...
export default {
  ...
  async asyncData (ctx) {
    const resp = await ctx.$axios.post('/order/getOrders')
    const { status, data: { code, list } } = resp
    console.log(list)
    if (status === 200 && code === 0 && list.length) {
      return {
        list: list.map((item) => {
          return {
            img: item.imgs.length ? item.imgs[0].url : '/img/logo.png',
            name: item.name,
            count: 1,
            total: item.total,
            status: item.status,
